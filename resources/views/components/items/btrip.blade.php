<x-accordion.wrap wire:key="{{ $btrip->id }}">
    <x-accordion.button>
        <div class="w-1/4">{{$btrip->event_date->format(config('app.date_format'))}}</div>
        <div class="w-1/4">{{$btrip->sum}}</div>
        <div class="w-1/4">{{$btrip->comment}}</div>
        <div class="w-1/4 flex justify-end gap-2">
            <x-buttons.edit wire:click="edit({{ $btrip->id }})" />
            <x-buttons.delete wire:click="confirmDelete({{ $btrip->id }})" />
        </div>
    </x-accordion.button>
    @if ($btrip->log->isNotEmpty())
    <x-accordion.content>
        <!-- Timeline -->
        <div>
            @foreach ( $btrip->log as $log )
            @if ($log)
            <!-- Item -->
            <x-timeline.wrap>
                <!-- Left Content -->
